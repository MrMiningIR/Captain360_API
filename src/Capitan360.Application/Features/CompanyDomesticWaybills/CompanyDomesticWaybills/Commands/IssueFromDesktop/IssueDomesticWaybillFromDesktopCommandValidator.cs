...
namespace Capitan360.Application.Features.CompanyDomesticWaybills.CompanyDomesticWaybills.Commands.IssueFromDesktop;

public class IssueDomesticWaybillFromDesktopCommandValidator : AbstractValidator<IssueCompanyDomesticWaybillFromDesktopCommand>
{
    public IssueDomesticWaybillFromDesktopCommandValidator()
    {
        RuleFor(x => x.No)
        ...
        RuleFor(x => x.CompanySenderBankPaymentNo)
            .NotNull().WithMessage("کد پرداخت بانک شرکت فرستنده نمی تواند خالی باشد است.")
            .MaximumLength(50).WithMessage("کد پرداخت بانک شرکت فرستنده نمی‌تواند بیشتر از 50 کاراکتر باشد")
            .When(x => x.CompanySenderBankPayment);
