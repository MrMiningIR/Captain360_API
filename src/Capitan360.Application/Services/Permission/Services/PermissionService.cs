...
namespace Capitan360.Application.Services.Permission.Services;
    ...
    PermissionCollectorService permissionCollector, IUnitOfWork unitOfWork, ILogger<PermissionService> logger, IMapper mapper) : IPermissionService
{
        ...
        try
        {
            ...
            foreach (var permission in permissionsData)
            {
                ...
                if (!exist)
                {
                    ...
                    await permissionRepository.AddPermissionToPermissionSource(new Domain.Entities.AuthorizationEntity.Permission
                    {
    ...
    public async Task<ApiResponse<List<Domain.Entities.AuthorizationEntity.Permission>>> GetDbPermissions(CancellationToken ct)
    {
        ...
        if (!permissions.Any())
            return ApiResponse<List<Domain.Entities.AuthorizationEntity.Permission>>.Error(400, "در دیتابیس مجوزی وجود ندارد");

        return ApiResponse<List<Domain.Entities.AuthorizationEntity.Permission>>.Ok(permissions.ToList());
    }
