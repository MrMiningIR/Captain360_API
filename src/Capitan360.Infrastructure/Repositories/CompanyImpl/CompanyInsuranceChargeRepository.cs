    ...
    : ICompanyInsuranceChargeRepository
{
    ...
    public async Task<CompanyInsuranceCharge?> GetCompanyInsuranceChargeById(int id, CancellationToken cancellationToken)
    {
        return await dbContext.CompanyInsuranceCharges
            .Include(a => a.CompanyInsurance)
            .Include(a => a.CompanyInsuranceChargePayments)
            .Include(a => a.CompanyInsuranceChargePaymentContentTypes)
            .SingleOrDefaultAsync(a => a.Id == id, cancellationToken);
    }

    public async Task<(IReadOnlyList<CompanyInsuranceCharge>, int)> GetMatchingAllCompanyInsuranceCharges(string? searchPhrase, int companyInsuranceId, int pageSize, int pageNumber, string? sortBy, SortDirection sortDirection, CancellationToken cancellationToken)
    {
