...
public class PermissionRepository(ApplicationDbContext dbContext, IUnitOfWork unitOfWork) : IPermissionRepository
{
    ...
    public async Task<List<ParentPermissionTransfer>> GetParentPermissions(CancellationToken cancellationToken)
    {
        return await dbContext.Permissions.AsNoTracking().Select(x =>
                new ParentPermissionTransfer { ParentName = x.ParentName, ParentDisplayName = x.ParentDisplayName, ParentCode = x.ParentCode }).Distinct()
            .ToListAsync(cancellationToken);
    }

    public async Task<List<Permission>> GetPermissionsByParentName(string parent, CancellationToken cancellationToken)
    {
        return await dbContext.Permissions.AsNoTracking().Where(x => x.ParentName.ToLower() == parent.Trim().ToLower()).Distinct().ToListAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var baseQuery = dbContext
            ...
            .Where(r => r.Name.ToLower().Contains(searchPhrase) ||
                        r.ParentName.ToLower().Contains(searchPhrase));
    ...
    public async Task<bool> HasPermissionAsync(string userId, string permission, CancellationToken cancellationToken)
    {
        ...
        //    .AnyAsync(gp => gp.Permission.Name == permission, cancellationToken);
    ...
    {
        var hasUserPermission = await dbContext.UserPermissions.AnyAsync(up => up.UserId == userId, cancellationToken);
