...
public class UserPermissionRepository(ApplicationDbContext dbContext, IUnitOfWork unitOfWork) : IUserPermissionRepository
{
    ...
    public async Task<(IReadOnlyList<UserPermission>, int)> GetMatchingAllUserPermissions(string userId, int pageSize, int pageNumber, CancellationToken cancellationToken)
    {
